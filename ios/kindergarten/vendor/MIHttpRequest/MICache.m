...
#if DEBUG
	#define CHECK_FOR_MICACHE_PLIST() if([key isEqualToString:@"MICache.plist"]) { \
		NSLog(@"MICache.plist is a reserved key and can not be modified."); \
		return; }
#else
	#define CHECK_FOR_MICACHE_PLIST() if([key isEqualToString:@"MICache.plist"]) return;
#endif



static NSString* _MICacheDirectory;

static inline NSString* MICacheDirectory() {
	if(!_MICacheDirectory) {
		NSString* cachesDirectory = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0];
		_MICacheDirectory = [[[cachesDirectory stringByAppendingPathComponent:[[NSProcessInfo processInfo] processName]] stringByAppendingPathComponent:@"MICache"] copy];
	}
	
	return _MICacheDirectory;
}

static inline NSString* cachePathForKey(NSString* key) {
	return [MICacheDirectory() stringByAppendingPathComponent:key];
}

static MICache* __instance;
...
@implementation MICache
@synthesize defaultTimeoutInterval;

+ (MICache*)currentCache {
	@synchronized(self) {
		if(!__instance) {
			__instance = [[MICache alloc] init];
...
- (id)init {
	if((self = [super init])) {
		NSDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:cachePathForKey(@"MICache.plist")];
		...
		[[NSFileManager defaultManager] createDirectoryAtPath:MICacheDirectory() 
...
- (void)saveCacheDictionary {
	@synchronized(self) {
		[cacheDictionary writeToFile:cachePathForKey(@"MICache.plist") atomically:YES];
	}
}
